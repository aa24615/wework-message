...
public class Message {
    ...
    public String getSavePath() {
        File file = new File(this.savePath + this.corpid);
        file.mkdirs();

        return this.savePath + this.corpid;
    }
    ...
    public ArrayList<Object> getList(long seq, int limit) throws Exception {
        ...
        for (int i = 0; i < chatdata.length(); i++) {
            ...
            long seqs = item.getLong("seq");

            String messageData = this.decryptData(encrypt_random_key, encrypt_chat_msg);

            list.add(this.getMessage(msgid, seqs, publickey_ver, messageData));
        }
